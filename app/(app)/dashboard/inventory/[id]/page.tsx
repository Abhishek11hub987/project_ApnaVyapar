...
export default function EditProductPage({ params }: { params: { id: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If no new image, we don't update imageUrl (it stays undefined/null, so we don't overwrite with null unless intended)
      // Actually, we should just not include image_url in the update if it hasn't changed.
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-3xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        <Link href="/dashboard/inventory" className="p-2 rounded-full hover:bg-gray-50 text-gray-400 hover:text-gray-900 transition-colors">
          <ArrowLeft size={20} />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900 mb-1">Edit Product</h1>
          <p className="text-gray-500">Update details for this product.</p>
        </div>
      </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex-1">
                  <label className="cursor-pointer bg-white text-gray-700 border border-gray-200 px-4 py-2 rounded-lg hover:bg-gray-50 inline-flex items-center gap-2 text-sm font-medium">
