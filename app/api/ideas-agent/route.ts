    ...
    const groqResponse = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: 'user',
            content: `First, VERIFY if the following title and description represent a genuine, sensible, and safe business idea. If it is spam, gibberish, harmful, or not a business idea, return a JSON object with exactly one field: { "rejected_reason": "Explanation" }. Do NOT return the full schema if rejected. 

If it IS a valid business idea, proceed to enrich it using the provided schema for the Indian market:
Title: ${title}
...
${profile?.city ? `Location context: ${profile.city}, India` : ''}`
    ...
    try {
      ...
      enrichedIdea = {
        ...
        real_example_url: null,
        market_size_note: null,
