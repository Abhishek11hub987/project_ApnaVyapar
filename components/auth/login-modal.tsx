...
import { Mail, ArrowRight, CheckCircle2, ShieldCheck, AlertCircle, X } from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-scale-in relative" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-8 text-center bg-teal-50/50 dark:bg-teal-900/20 border-b border-teal-100 dark:border-teal-800/50 relative">
          <div className="w-14 h-14 bg-teal-600 rounded-2xl mx-auto flex items-center justify-center shadow-lg shadow-teal-600/30 mb-4 rotate-3">
            <span className="text-white font-black text-xl tracking-tighter -rotate-3">AV</span>
          </div>
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-2 tracking-tight">Apna Vyapar</h2>
          ...
          {step === 'email' ? (
            <div className="space-y-6">
              <button 
                ...
                disabled={loading}
                className="w-full flex items-center justify-center gap-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 font-semibold py-3.5 px-4 rounded-xl transition-all"
              >
