...
function ChatContent() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] md:h-[600px] bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
      {/* Header */}
      <div className="bg-white border-b border-slate-200 px-4 py-3 flex justify-between items-center z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-slate-800 leading-tight">Vyapar Mitra</h2>
            <p className="text-xs text-slate-500">Your AI Business Advisor</p>
          </div>
        </div>
        <button 
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6">
        {messages.length === 0 ? (
          <div className="text-center text-slate-500 mt-8 md:mt-12 animate-in fade-in duration-700">
            <div className="w-16 h-16 bg-teal-100 text-teal-700 rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm">
              <Bot size={32} />
            </div>
            <p className="font-semibold text-slate-800 mb-1 text-lg">Welcome! I'm Vyapar Mitra.</p>
      ...
      <div className="bg-white border-t border-slate-200 p-4">
        <form onSubmit={handleSubmit} className="relative flex items-center max-w-4xl mx-auto">
          <input
            ...
            className="w-full bg-slate-100 border border-slate-200 rounded-full pl-5 pr-14 py-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all"
            disabled={isLoading}
          />
          <button 
            type="submit" 
            disabled={!input.trim() || isLoading}
            className="absolute right-2 w-10 h-10 flex items-center justify-center bg-teal-700 text-white rounded-full disabled:opacity-50 disabled:bg-slate-300 hover:bg-teal-600 transition-colors"
          >
