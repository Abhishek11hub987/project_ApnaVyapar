...
export default function IdeaCard({ idea, isCommunity = false }: { idea: any, isCommunity?: boolean }) {
  ...
  const getInitialSrc = () => {
    // Force local images for curated ideas to bypass ISP Unsplash blocks
    if (!isCommunity && idea.slug) {
      return `/images/ideas/${idea.slug}.jpg`;
    }
    return idea.image_url || CATEGORY_IMAGES[idea.category] || DEFAULT_IMAGE;
  };
  ...
  const handleImgError = () => {
    // If the local slug image or DB image fails, fallback to Unsplash category images
    if (!useFallback && imgSrc !== CATEGORY_IMAGES[idea.category]) {
      setUseFallback(true);
      setImgSrc(CATEGORY_IMAGES[idea.category] || DEFAULT_IMAGE);
    } else {
      // If the fallback Unsplash image also fails (ISP block), show gradient
      setImgError(true);
    }
  };

  return (
    <div className="bg-white border border-gray-100 rounded-3xl overflow-hidden flex flex-col group shadow-subtle hover:shadow-elevated hover:-translate-y-1.5 transition-all duration-300">
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <Link
          ...
          className="w-full block text-center text-sm font-bold bg-white text-gray-900 border border-gray-200 py-3 rounded-xl hover:border-gray-900 hover:shadow-md transition-all duration-300"
        >
          {t('card.viewDetails')}
        </Link>
      </div>
