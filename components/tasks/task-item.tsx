...
export default function TaskItem({ task, onStatusChange }: { task: ChecklistTask, onStatusChange: (id: string, status: string) => void }) {
  ...
  return (
    <div className={`p-5 rounded-2xl border mb-4 transition-all duration-300 hover:shadow-md ${
      isCompleted 
        ? 'bg-teal-50/40 border-teal-100 opacity-75 hover:opacity-100' 
        : isInProgress 
          ? 'bg-gradient-to-br from-amber-50/50 to-white border-amber-200 shadow-sm' 
